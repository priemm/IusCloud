...
<button onclick="alert('Google Login se configura después')">Ingresar con Google</button>
</div>
</div>
`;
}
...
if (!data || !data.ok) return alert(data?.error || "Error al registrar");
saveSession(data);
renderDashboard();
}
...
if (!data || !data.ok) return alert(data?.error || "Error al ingresar");
saveSession(data);
renderDashboard();
}
...
${(tipos || []).map(t => `<option value="${t.nombre || ""}">${t.nombre || "Sin nombre"}</option>`).join("")}
</select>
...
return m.tipo === "Cobro" ? acc + monto : acc - monto;
}, 0);
...
<div class="item">
<div>
<strong>${t.titulo || "Sin título"}</strong>
<p>${t.estado || "Pendiente"} · Vence: ${t.vencimiento || "-"}</p>
<p>${t.descripcion || ""}</p>
...
<div class="item">
<div>
<strong>${m.tipo || "Movimiento"}: $${m.monto || 0}</strong>
<p>${m.fecha || ""} · ${m.concepto || ""}</p>
</div>
</div>
`).join("") || "<p>Sin movimientos todavía.</p>"}
</div>
...
${(modelos || []).map(m => `<option value="${m._id}">${m.titulo || "Sin título"}</option>`).join("")}
</select>
...
<div class="item">
<div>
<strong>${h.tipo || "Movimiento"}</strong>
...
if (!modelo) return alert("Elegí un modelo");
...
method: "POST",
body: JSON.stringify({
nombre: tpNombre.value,
descripcion: tpDesc.value
})
});
...
method: "POST",
body: JSON.stringify({
titulo: moTitulo.value,
tipo: moTipo.value,
contenido: moContenido.value
})
});
